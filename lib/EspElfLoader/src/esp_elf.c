...
static int esp_elf_load_section(esp_elf_t* elf, const uint8_t* pbuf) {
  ...
  for (uint32_t i = 0; i < ehdr->shnum; i++) {
    ...
    if (stype(&shdr[i], SHT_PROGBITS) && sflags(&shdr[i], SHF_ALLOC)) {
      if (sflags(&shdr[i], SHF_EXECINSTR) && !strcmp(ELF_TEXT, name)) {
        ESP_LOGD(TAG, ".text   sec addr=0x%08x size=0x%08x offset=0x%08x", shdr[i].addr, shdr[i].size, shdr[i].offset);
        ...
        ESP_LOGD(TAG, ".text   offset is 0x%lx size is 0x%x", elf->sec[ELF_SEC_TEXT].offset,
                 elf->sec[ELF_SEC_TEXT].size);
      } else if (sflags(&shdr[i], SHF_WRITE) && !strcmp(ELF_DATA, name)) {
        ESP_LOGD(TAG, ".data   sec addr=0x%08x size=0x%08x offset=0x%08x", shdr[i].addr, shdr[i].size, shdr[i].offset);

        elf->sec[ELF_SEC_DATA].v_addr = shdr[i].addr;
        elf->sec[ELF_SEC_DATA].size = shdr[i].size;
        elf->sec[ELF_SEC_DATA].offset = shdr[i].offset;

        ESP_LOGD(TAG, ".data   offset is 0x%lx size is 0x%x", elf->sec[ELF_SEC_DATA].offset,
                 elf->sec[ELF_SEC_DATA].size);
      } else if (!strcmp(ELF_RODATA, name)) {
        ESP_LOGD(TAG, ".rodata sec addr=0x%08x size=0x%08x offset=0x%08x", shdr[i].addr, shdr[i].size, shdr[i].offset);
        ...
        ESP_LOGD(TAG, ".rodata offset is 0x%lx size is 0x%x", elf->sec[ELF_SEC_RODATA].offset,
                 elf->sec[ELF_SEC_RODATA].size);
      } else if (!strcmp(ELF_DATA_REL_RO, name)) {
        ESP_LOGD(TAG, ".data.rel.ro sec addr=0x%08x size=0x%08x offset=0x%08x", shdr[i].addr, shdr[i].size,
                 shdr[i].offset);
        ...
        ESP_LOGD(TAG, ".data.rel.ro offset is 0x%lx size is 0x%x", elf->sec[ELF_SEC_DRLRO].offset,
                 elf->sec[ELF_SEC_DRLRO].size);
      }
    } else if (stype(&shdr[i], SHT_NOBITS) && sflags(&shdr[i], SHF_ALLOC | SHF_WRITE) && !strcmp(ELF_BSS, name)) {
      ESP_LOGD(TAG, ".bss    sec addr=0x%08x size=0x%08x offset=0x%08x", shdr[i].addr, shdr[i].size, shdr[i].offset);

      elf->sec[ELF_SEC_BSS].v_addr = shdr[i].addr;
      elf->sec[ELF_SEC_BSS].size = shdr[i].size;
      elf->sec[ELF_SEC_BSS].offset = shdr[i].offset;

      ESP_LOGD(TAG, ".bss    offset is 0x%lx size is 0x%x", elf->sec[ELF_SEC_BSS].offset, elf->sec[ELF_SEC_BSS].size);
    }
  }
  ...
  if (!elf->ptext) {
    ESP_LOGE(TAG, "Failed to malloc %" PRIu32 " bytes for text section", (uint32_t)elf->sec[ELF_SEC_TEXT].size);
    return -ENOMEM;
  }

  size = elf->sec[ELF_SEC_DATA].size + elf->sec[ELF_SEC_RODATA].size + elf->sec[ELF_SEC_BSS].size +
         elf->sec[ELF_SEC_DRLRO].size;
  if (size) {
...
static int esp_elf_load_segment(esp_elf_t* elf, const uint8_t* pbuf) {
  ...
  for (int i = 0; i < ehdr->phnum; i++) {
    ...
    if (first_segment == true) {
      ...
      if (phdr[i].vaddr < vaddr_e) {
        ESP_LOGE(TAG, "Invalid segment[%d], should not overlap, vaddr: 0x%x, vaddr_e: 0x%x", i, phdr[i].vaddr, vaddr_e);
        return -EINVAL;
      }
      ...
      if (vaddr_e < phdr[i].vaddr) {
        ESP_LOGE(TAG, "Invalid segment[%d], address overflow, vaddr: 0x%x, vaddr_e: 0x%x", i, phdr[i].vaddr, vaddr_e);
        return -EINVAL;
      }
    }
  ...
  for (int i = 0; i < ehdr->phnum; i++) {
    if (phdr[i].type == PT_LOAD) {
      ...
      ESP_LOGD(TAG, "Copy segment[%d], mem_addr: %p, size: 0x%08x", i,
               (void*)((uint8_t*)elf->psegment + phdr[i].vaddr - vaddr_s), phdr[i].filesz);
    }
  }
    ...
    if (stype(&shdr[i], SHT_RELA)) {
      ...
      if (strcmp((const char*)(shstrab + shdr[i].name), ELF_DYNSYM) == 0) {
        ...
        for (j = 0; j < shdr[i].size / sizeof(elf32_sym_t); j++) {
          if ((ELF_ST_BIND(symtab[j].info) == STB_GLOBAL) && (ELF_ST_TYPE(symtab[j].info) == STT_FUNC)) {
            elf->num++;
          }
        }
...
#if CONFIG_ELF_LOADER_BUS_ADDRESS_MIRROR
            elf->symtab[num].addr = (void*)(elf->ptext + symtab[j].value - elf->sec[ELF_SEC_TEXT].v_addr);
#else
