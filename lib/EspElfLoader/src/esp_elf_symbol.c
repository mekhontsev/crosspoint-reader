 ...
 */
static int* s_picolibc_errno_func(void) { return &errno; }

static struct _reent* s_picolibc_getreent_func(void) { return NULL; }
...
/* libc (newlib / picolibc) */
    ...
    ESP_ELFSYM_EXPORT(__errno), ESP_ELFSYM_EXPORT(__getreent), ESP_ELFSYM_EXPORT(__locale_ctype_ptr),
#else
    ESP_ELFSYM_EXPORT(__errno), ESP_ELFSYM_EXPORT(__getreent), ESP_ELFSYM_EXPORT(_ctype_),
#endif
...
static const struct esp_elfsym g_esp_espidf_elfsyms[] = {
    ...
    ESP_ELFSYM_EXPORT(lwip_bind), ESP_ELFSYM_EXPORT(lwip_setsockopt), ESP_ELFSYM_EXPORT(lwip_socket),
    ESP_ELFSYM_EXPORT(lwip_listen), ESP_ELFSYM_EXPORT(lwip_accept), ESP_ELFSYM_EXPORT(lwip_recv),
    ESP_ELFSYM_EXPORT(lwip_recvfrom), ESP_ELFSYM_EXPORT(lwip_send), ESP_ELFSYM_EXPORT(lwip_sendto),
    ESP_ELFSYM_EXPORT(lwip_connect),

    /* arpa/inet.h */

    ESP_ELFSYM_EXPORT(ipaddr_addr), ESP_ELFSYM_EXPORT(lwip_htons), ESP_ELFSYM_EXPORT(lwip_htonl),
...
uintptr_t elf_find_sym_default(const char* sym_name) {
  ...
  /* CrossPoint embeds one SD module and supplies its host resolver directly.
   * Module-to-module dlmod lookup is intentionally unavailable. */
  return 0;
}
